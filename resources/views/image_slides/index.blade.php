                ...
                <div class="card card-md">
                    <div class="card-body">
                        <div class="row align-items-center">
                            <div class="col">
                                <h2 class="h3">Image Slides List.</h2>
                            ...
                            <div class="col-auto">
                                <button type="button" class="btn btn-primary btn-create" data-bs-toggle="modal"
                                    data-bs-target="#modal-image-slide">
                ...
                <div class="col-12">
                    <div class="card">
                                ...
                                <tbody>
                                    @foreach ($imageSlide as $i => $item)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="{{ $item->status ? 'badge bg-blue' : 'badge bg-danger' }}">{{ $item->status ? 'Published' : 'Draft' }}</span> 
                                            </td>
                                            <td>
                                                <button type="button" class="btn btn-primary btn-icon btn-edit"
                                                    data-bs-toggle="modal" data-bs-target="#modal-image-slide"
                                                    data-id="{{ $item->id }}">
                        ...
                        <div class="card-footer d-flex align-items-center">
                            <p class="m-0 text-muted">Showing <span>1</span> to <span>{{ $imageSlide->count() }}</span> of
                                <span>{{ $imageSlide->total() }}</span> entries</p>
                            @if ($imageSlide->lastPage() > 1)
                                <ul class="pagination m-0 ms-auto">
                                    @if ($imageSlide->currentPage() != 1 && $imageSlide->lastPage() >= 5)
                                        <li class="page-item me-3">
                                            <a href="{{ $imageSlide->url($imageSlide->url(1)) }}" aria-label="Previous"
                                                class="page-link">
                                                <span aria-hidden="true">First</span>
                                            </a>
                                        </li>
                                    @endif
                                    {{-- @if ($imageSlide->currentPage() != 1) --}}
                                    <li class="page-item">
                                        <a href="{{ $imageSlide->url($imageSlide->currentPage() - 1) }}" class="page-link me-2"
                                            aria-label="Previous">
                                    ...
                                    {{-- @endif --}}
                                    @for ($i = max($imageSlide->currentPage() - 2, 1); $i <= min(max($imageSlide->currentPage() - 2, 1) + 4, $imageSlide->lastPage()); $i++)
                                        @if ($imageSlide->currentPage() == $i)
                                            <li class="active page-item">
                                            @else
                                            <li>
                                        @endif
                                        <a href="{{ $imageSlide->url($i) }}" class="page-link">{{ $i }}</a>
                                        </li>
                                    @endfor
                                    {{-- @if ($imageSlide->currentPage() != $imageSlide->lastPage()) --}}
                                    <li class="page-item">
                                        <a href="{{ $imageSlide->url($imageSlide->currentPage() + 1) }}" class="page-link"
                                            aria-label="Next">
                                    ...
                                    {{-- @endif --}}
                                    @if ($imageSlide->currentPage() != $imageSlide->lastPage() && $imageSlide->lastPage() >= 5)
                                        <li class="page-item mx-3">
                                            <a href="{{ $imageSlide->url($imageSlide->lastPage()) }}" class="page-link"
                                                aria-label="Next">
...
@endsection
@section('modals')
    <div class="modal modal-blur fade" id="modal-image-slide" tabindex="-1" role="dialog" aria-hidden="true">
                    ...
                    <div class="modal-body">
                        <div class="row mb-3 align-items-end">
                            ...
                            <div class="col">
                                ...
                                <select type="text" name="status" id="select-tags content_type" class="form-select"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '.btn-edit', function() {
                var id = $(this).data('id');
                $.get("{{ url('/image-slides') }}" + '/' + id + '/edit', function(data) {
